...
void USB_ISR () interrupt 8
{
   ...
   UREAD_BYTE(CMINT, bCommonInt);//检测复位信号，读到bCommonInt中
   UREAD_BYTE(IN1INT, bInInt);
   UREAD_BYTE(OUT1INT, bOutInt);

   // Check for reset interrupt
   if (bCommonInt & rbRSTINT) //如果有复位信号
   {
      // Call reset handler
     USBReset();
      M_State = ST_WAIT_DEV;
...
void Endpoint0 ()
{
   ...
   {                                      // Indicate setup end serviced
      UWRITE_BYTE(E0CSR, rbSSUEND);		//软件应在处理完 SETUP 结束事件（SUEND）后将该位置‘1’
